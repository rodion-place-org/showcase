...
class BuildTests(unittest.TestCase):
    def test_build_writes_required_pages(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            self.assertIn("Start from your task", projects_index_text)
            self.assertIn("Prepare for a reporting clock", projects_index_text)
            self.assertIn("Screen an OSS bounty", projects_index_text)
            self.assertIn("Explore the local tools", projects_index_text)
            ...
            self.assertIn("0 safe contribution targets", bounty_text)
            self.assertIn('<strong><time datetime="2026-09-13T20:49:11Z">13 Sep 2026 · 20:49 UTC</time> · 6 qualified payers', bounty_text)
            self.assertIn("Reading matrix: six qualified payers at 360-day window", bounty_text)
            self.assertIn("Completed payout cards observed in the trailing 360 days", bounty_text)
            self.assertIn("https://algora.io/activepieces/bounties?status=completed", bounty_text)
            self.assertIn("This matrix measures payout recency and open-work availability", bounty_text)
            self.assertIn("across six organisations", bounty_text)
            self.assertNotIn("across seven organisations", bounty_text)
            self.assertIn("Algora completed pages (6 orgs)", bounty_text)
            self.assertIn("Public sources for this reading", bounty_text)
            self.assertIn("https://api.github.com/repos/Mudlet/Mudlet/issues/5310", bounty_text)
            self.assertIn("Read the current evidence in order", bounty_text)
            self.assertIn("Open the GitHub issue API records", bounty_text)
            ...
            self.assertIn("Official agent-route inventory reading", bounty_text)
            self.assertIn("OnlyDust rail check: no current contribution route", bounty_text)
            self.assertIn("The OnlyDust chapter closes here", bounty_text)
            self.assertIn("stopped accepting external contributions", bounty_text)
            self.assertIn("https://www.onlydust.com/", bounty_text)
            self.assertIn("9 agent-eligible listings", bounty_text)
            self.assertIn("all nine were expired", bounty_text)
            self.assertIn("does not establish a live opportunity", bounty_text)
            ...
            self.assertIn('datetime="2026-09-13T20:40:00Z"', bounty_text)
